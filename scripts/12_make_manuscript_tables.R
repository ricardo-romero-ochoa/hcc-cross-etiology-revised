...
if (file.exists("results/tables/meta_analysis_with_heterogeneity.tsv")) {
  ...
  # Compatibility alias requested by final repository audit / reviewer-facing manifest
  readr::write_csv(tbl1, "manuscript/tables/Table1_meta_summary.csv")
...
if (file.exists("results/tables/gse121248_hbv_injury_topN_extended_manuscript_summary.tsv")) {
  hbv <- readr::read_tsv("results/tables/gse121248_hbv_injury_topN_extended_manuscript_summary.tsv", show_col_types = FALSE) |>
    ...
    mutate(
      ...
      Role = dplyr::case_when(analysis_role %in% c("compact_primary", "compact_primary_candidate") ~ "Primary compact", TRUE ~ "Sensitivity"),
  ...
  # Compatibility alias requested by final repository audit / reviewer-facing manifest
  readr::write_csv(hbv, "manuscript/tables/Table2_HBV_injury_regression.csv")
...
if (file.exists("results/tables/tcga_lihc_survival_cox_models.tsv")) {
  ...
  # Compatibility alias requested by final repository audit / reviewer-facing manifest
  readr::write_csv(cox, "manuscript/tables/Table3_TCGA_Cox.csv")
